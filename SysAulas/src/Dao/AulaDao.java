...
public class AulaDao {
    ...
    /*public static HashSet<Integer> findBetweenDias(String nombreDia, java.sql.Date fecha, Time hora, Integer duracion) {
        System.out.println("fecha: "+fecha);
        System.out.println("hora: "+hora);
        ...
        try {
            tx = sesion.beginTransaction();
            fechas = sesion.createQuery("FROM Fecha f WHERE f.dia.nombreDia ='"+nombreDia+"'").list();
            tx.commit();
        } catch (HibernateException e) {
            if (tx != null) {
                tx.rollback();
